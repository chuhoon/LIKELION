...
- 가비지 컬랙터는 무엇이고 JS에서는 어떻게 동작하나요?
  -> 가비지 컬렉터는 더 이상 사용하지 않는 메모리를 발견해 해제하는 역할을 하는 것입니다.
...
- 서버사이드 랜더링과 클라이언트 사이드 랜더링의 차이에 대해 설명해주세요.
  ...
  클라이언트 사이드 랜더링의 경우 사용성이 아주 좋습니다. 마치 하나의 프로그램을 사용하느낌의 부드러움을 느낄 수 있습니다. 하지만 너무 많은 리소스를 다운받아야 하고(대량의 JS파일) 렌더링을 위한 페이지의 조립 자체를 클라이언트에서 처리하기 때문에 초기 화면을 로딩하는 속도가 느림니다.

## 2022-06-22 면접 질문

- 왜 당신을 프론트엔드 개발자로 뽑아야 하나요?
  -> 저를 뽑아야 하는 이유를 3가지로 말해보겠습니다.

  첫 번째는 흡수력이 좋은 사람이기 때문입니다. 4개월 안에 기능이 동작하는 프로젝트를 3개 진행하고, 컨퍼런스 연사자, 책 출간을 할 만큼 성장이 빠른 인재라고 스스로 생각합니다. 성장이 빠르다는 것은 곧 흡수력이 빠르다고 생각하며 회사 업무에 적응을 잘 할 수 있는 사람이라는 것을 반증한다고 생각합니다.

  두 번째는 프론트엔드 개발자로 일하기 적합한 기술스택을 가지고 있다는 점입니다. 제가 사용할 수 있는 기술스택으로 HTML/CSS, Vanila JS, React가 있습니다. 이는 프론트엔드 개발자로 일하기에 적합한 기술 스택을 가지고 있다고 말할 수 있습니다.

  세 번째는 협업에 능통한편이라고 말할 수 있습니다. 과거 다수의 창업경험과 일 경험으로 기획에 대해 이해도가 있습니다. 그리고 디자인 툴을 약간 다룰 수 있습니다. 타 부서에 대한 이해도는 협업에 있어 의사소통을 하는 데 원할할 수 있다고 생각합니다.

- 최근에 배운 프론트엔드 관련 기술은 무엇인가요?

## 2022-06-27 면접 질문

- section요소와 article 요소의 사용 용도에 대해 설명해라
  -> section 요소는 관련있는 내용을 묶는 용도이고 article은 관련 있는 내용 속에서 독립적으로 구성된 글을 묶는 역할이다.
  -> best 답변 : 둘 다 HTML 문서를 구획짓는 nav 나 body 요소같은 sectioning 요소에 속하며, section 요소는 제목을 가지고 있는 비슷한 주제의 컨텐츠의 그룹을 나타낼때, article의 경우 문서 내의 하나의 완전한 독립적인 컨텐츠를 명시할 때 사용합니다.
  신문으로 따진다면, 경제 면, 스포츠 면 과 같은 큰 주제를 section으로, 그 안의 독립적인 기사들을 article 로 볼 수 있을것 같습니다.

- null과 undefined에 대해 설명해주세요
  -> null은 어떤 값이 비어있음을 표현합니다. undefined는 값이 지정되지 않은 경우를 의미하고, null은 해당 변수가 어떤 객체도 가리키고 있지 않는 것을 의미합니다.
  -> best 답변 : null 은 비어 있는, 존재하지 않는 값을 의미합니다. undefined는 변수가 정의 되어 있지만 아무런 값도 할당 받지 않은 상태를 의미합니다.  
  null 의 typeof 는 object를 반환합니다.  undefined의 typeof 는 undefined를 반환합니다.

- input type="button"과 button 태그의 차이점을 설명하세요.
  -> input타입은 스스로 닫힌 태그가 달리기에 자식 요소를 추가할 수 없지만 button 태그는 그렇지 않기에 자식요소가 추가 가능합니다.
  -> best 답변 : 둘 다 누를 수 있는 버튼을 의미하는것은 동일하지만 button 태그의 경우 닫는 태그가 존재한다는 차이가 있습니다. 이를 통해 안에 추가적인 컨텐츠를 넣을 수 있기 때문에 더 풍부한 내용을 담을 수 있고 스타일을 꾸미는데 있어서 더 높은 자유도를 가지고 있습니다.

- 렉시컬 스코프란 무엇인가요?
  -> 우리가 언어를 만들 때 렉시컬 스코프로 만들겠어! 라고 생각을 했다면 이런 환경을 구현해야겠지요?
  렉시컬 스코프를 구현하기 위해 만들어진 객체가 바로 렉시컬 환경입니다. 자바스크립트 엔진이 우리가 작성한 코드를 실행시킬 때, 변수와 함수의 스코프를 기록하는 객체를 만듭니다. 이런 기록들을 저장하는 객체가 렉시컬 환경입니다. (마치 HTML코드를 통해 DOM 트리로 만드는 것 처럼 너의 외부에는 어떤 변수와 함수들이 있고, 너의 안에는 어떤 변수와 함수들이 있구나 하는 족보라고 이해하시면 됩니다.) 렉시컬 환경 객체는 두 부분으로 구성됩니다.

  1. 환경 레코드(Environment Record) – this 값을 포함한 모든 지역 변수를 프로퍼티로 저장하고 있는 객체입니다.
  2. 외부 렉시컬 환경(외부 코드)에 대한 참조

  -> 자바스크립트 엔진이 우리가 작성한 코드를 실행시킬 때, 변수와 함수의 스코프를 기록하는 객체를 만듭니다.
  이런 기록들을 저장하는 객체가 렉시컬 환경입니다.

- 요소를 배치하는 방법(relative, fixed, absolute, static) 간의 차이는 무엇인가요?
  -> relative의 기준은 자기 자신, fixed는 뷰포트 기준으로 배치, absolute는 부모 요소를 기준으로 배치, static은 기준이 존재하지 않습니다.

- 점진적 향상 기법에 대해 설명해주세요
  ->

- 우아한 성능저하에 대해 설명해주세요
  -> 최신 브라우저에서 최신 기술의 도입으로 최상의 사용자 경험을 제공하지만,
  구형 브라우저에서는 더 낮은 수준의 사용자 경험으로 단계적으로 저하될 수 있도록 하는 방법입니다.
